 ...
 * multiple sites = multiple registered providers. The provider talks to the engine
 * through [LynxChanApi] and maps all transport failures to the [ProviderException] contract,
 * so callers never see Ktor types.
 */
class LynxChanProvider(
    ...
    private fun ResponseException.retryAfterSeconds(): Long? = response.headers[HttpHeaders.RetryAfter]?.toLongOrNull()
