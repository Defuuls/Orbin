...
and releases work. Current as of **v68 (Deneb)**. See also
...
2. Tag and push. Tags are `v<number>-<Codename>`, and the tag must be **annotated** — the
   ...
   ```bash
   git tag -a v68-Deneb -m "Deneb"
   git push origin v68-Deneb
   ```
...
Betelgeuse, Procyon, Achernar, Hadar, Acrux, Aldebaran, Antares, Spica, Pollux, Deneb.
