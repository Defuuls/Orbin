...
and releases work. Current as of **v80 (Merak)**. See also
...
2. Tag and push. Tags are `v<number>-<Codename>`, and the tag must be **annotated** — the
   ...
   ```bash
   git tag -a v80-Merak -m "Merak"
   git push origin v80-Merak
   ```
...
Bellatrix, Elnath, Alnair, Peacock, Avior, Alkaid, Mirfak, Dubhe, Alioth, Mizar, Merak.
