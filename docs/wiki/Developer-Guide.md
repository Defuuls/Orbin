...
and releases work. Current as of **v79 (Mizar)**. See also
...
2. Tag and push. Tags are `v<number>-<Codename>`, and the tag must be **annotated** — the
   ...
   ```bash
   git tag -a v79-Mizar -m "Mizar"
   git push origin v79-Mizar
   ```
...
Bellatrix, Elnath, Alnair, Peacock, Avior, Alkaid, Mirfak, Dubhe, Alioth, Mizar.
