...
(bookmarks, history). It also holds `BoardPreferencesStore` (followed and favourite boards, feed
limits) and `SettingsStore` (the app settings), both in a Preferences DataStore under the keys
Android has always used. Android's `SettingsRepositoryImpl` delegates to both over its encrypted
settings store, and iOS uses a DataStore file of its own. Only the schema is shared. `data` opens it on Android, encrypted with
