...
sealed class DataError(
    ...
    ) : DataError("No network connection", cause)

    class Timeout(
        cause: Throwable? = null,
    ) : DataError("The request timed out", cause)

    class NotFound(
        detail: String = "Not found",
        cause: Throwable? = null,
    ) : DataError(detail, cause)

    class RateLimited(
        val retryAfterSeconds: Long? = null,
    ) : DataError("Too many requests")

    class Server(
        val code: Int,
        cause: Throwable? = null,
    ) : DataError("Server error ($code)", cause)

    class Parse(
        cause: Throwable? = null,
    ) : DataError("Could not read the response", cause)

    class Unknown(
        cause: Throwable? = null,
    ) : DataError("Something went wrong", cause)
}

inline fun <T, R> OrbinResult<T>.map(transform: (T) -> R): OrbinResult<R> =
    when (this) {
...
inline fun <T> OrbinResult<T>.onSuccess(action: (T) -> Unit): OrbinResult<T> =
    apply {
        if (this is OrbinResult.Success) action(data)
    }

inline fun <T> OrbinResult<T>.onFailure(action: (DataError) -> Unit): OrbinResult<T> =
    apply {
